...
import '../../bloc/chat_service.dart';
import '../../bloc/chatting_provider.dart';
import 'chat_page.dart';
import 'pick_up_call_screen.dart';
...
class _ChatScreenState extends State<ChatScreen> {
                                ...
                                : Expanded(
                                    child: ListView.builder(
                                      padding: const EdgeInsets.symmetric(
                                          vertical: 8),
                                      itemCount: chattingProvider
                                          .searchIndexList.length,
                                      itemBuilder: (context, index) {
                                        var result = chattingProvider
                                            .searchIndexList[index];
                                        return ListTile(
                                            dense: true,
                                            onTap: () {
                                              Navigator.push(
                                            ...
                                            title: Text(
                                              chattingProvider
                                                  .searchiteams[result]["name"],
                                              style: Theme.of(context)
                                                  .textTheme
                                                  .bodyMedium!,
                                            ),
                                            subtitle: Text(
                                                chattingProvider.searchiteams[result]
                                                    ["message"],
                                                style: Theme.of(context)
                                                    .textTheme
                                                    .bodySmall),
                                            trailing: Text(
                                                DateFormat('hh:mm a')
                                                    .format(DateTime.fromMicrosecondsSinceEpoch(chattingProvider.searchiteams[result]["timestamp"].microsecondsSinceEpoch))
                                                    .toString(),
        ...
        builder: (context, snapshot) {
          ...
          if (snapshot.connectionState == ConnectionState.waiting) {
            return const ListTile(
              ...
              leading: CommonShimmerWidget(height: 50, width: 50, radius: 50),
              title: Row(
                children: [
                  CommonShimmerWidget(height: 10, width: 50, radius: 10),
                ],
              ),
              subtitle: Row(
                children: [
                  CommonShimmerWidget(height: 10, width: 30, radius: 10),
                ],
              ),
              trailing: CommonShimmerWidget(height: 10, width: 30, radius: 10),
            );
          } else {
            return ListView(
  ...
  Widget _buildMessageiteam(document, String email, String uid, String proPic,
      int legnth, var snapshot) {
    ...
    onDidReceiveNotificationResponse:
      ...
      if (payload != null) {
        ...
        if (data["vcId"] == "null" && data["Audio"] == "null") {
          ...
          navigatorKey.currentState?.push(MaterialPageRoute(
            builder: (context) =>
                PickUpCallScreen(userData: data, isAudio: false),
          ));
        } else if (data["Audio"] != null) {
          navigatorKey.currentState?.push(MaterialPageRoute(
            builder: (context) =>
                PickUpCallScreen(userData: data, isAudio: true),
          ));
        }
      }
    },
