...
class OnBordingProvider with ChangeNotifier {
                    ...
                    RichText(
                      ...
                      TextSpan(
                          text: AppLocalizations.of(context)
                                  ?.translate("We have sent the OTP to") ??
                              "We have sent the OTP to",
                          style: Theme.of(context).textTheme.bodyMedium),
                      TextSpan(
                          text: " +$ccode ${mobileNumber.text}",
                          style: Theme.of(context).textTheme.bodyMedium),
                    ])),
  ...
  searchLanguage(s) {
    ...
    for (int i = 0; i < languageModel!.languagelist!.length; i++) {
      if (languageModel!.languagelist![i].title
        ...
        final ids =
            languageModel!.languagelist!.map<String>((e) => e.id!).toSet();
  ...
  searchHobie(s) {
    ...
    for (int i = 0; i < getInterestModel!.interestlist!.length; i++) {
      if (getInterestModel!.interestlist![i].title
          ...
          .contains(s.toLowerCase())) {
        final ids =
            getInterestModel!.interestlist!.map<String>((e) => e.id!).toSet();
  ...
  searchReligion(s) {
    ...
    for (int i = 0; i < religionModel!.religionlist!.length; i++) {
      if (religionModel!.religionlist![i].title
          .toString()
          .toLowerCase()
          .contains(s.toLowerCase())) {
        final ids =
            religionModel!.religionlist!.map<String>((e) => e.id!).toSet();
