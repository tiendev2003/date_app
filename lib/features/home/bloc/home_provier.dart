...
import '../../chatting/bloc/chatting_provider.dart';
import '../../edit_profile/bloc/editprofile_provider.dart';
import '../../map/data/models/map_model.dart';
...
class HomeProvider extends ChangeNotifier {
  ...
  mapOnChange(value) {
    for (int i = 0; i < mapModel.profilelist!.length; i++) {
      int index = i + 1;
      if (mapDataList[index]["id"] ==
          "${mapModel.profilelist![value].profileId}") {
        ...
        mapDataList[index]["widget"] = Container(
          ...
          decoration: BoxDecoration(
              ...
              image: DecorationImage(
                  image: NetworkImage(
                      "${Config.baseUrl}${mapModel.profilelist![i].profileImages![0]}"),
    ...
    Future.delayed(
      ...
      () {
        mapMarkers();
        updatePosition(mapModel.profilelist![value].profileLat!,
            mapModel.profilelist![value].profileLongs!);
        notifyListeners();
      },
    );
  }
  ...
  mapData(
    ...
    try {
      ...
      var response = await _api.sendRequest
          .post("${Config.baseUrlApi}${Config.mapInfo}", data: data);
        ...
        if (response.data["Result"] == "true") {
          ...
          for (int a = 0; a < mapModel.profilelist!.length; a++) {
            Map data = {
              ...
              "gkey": GlobalKey(),
              "position": LatLng(
                  double.parse(mapModel.profilelist![a].profileLat.toString()),
                  double.parse(
                      mapModel.profilelist![a].profileLongs.toString())),
              "widget": Container(
                height: 100,
                width: 100,
                alignment: Alignment.center,
                decoration: BoxDecoration(
                    shape: BoxShape.circle,
                    border: Border.all(
                        color: a == 0 ? AppColors.appColor : AppColors.black,
                        width: 5),
                    image: DecorationImage(
                        image: NetworkImage(
  ...
  notificationApi() async {
    ...
    try {
      var response = await _api.sendRequest
          .get("${Config.baseUrlApi}${Config.notificationList}", data: data);
  ...
  loadDataFrorMap(context) async {
    ...
    Map data = {
      ...
      "widget": SvgPicture.asset("assets/icons/mappincurrent.svg",
  ...
  Future getmarkers(Map data, index) async {
    RenderRepaintBoundary boundary =
        data["gkey"].currentContext.findRenderObject();
    ...
    return Marker(
        ...
        icon: BitmapDescriptor.bytes(pngBytes.buffer.asUint8List()));
  }
  ...
  Future cancleButton(state, context) async {
    ...
    animation = Tween<Offset>(begin: Offset.zero, end: const Offset(-0.80, 0.1))
        .animate(controller);
    notifyListeners();
                  ...
                  MainButton(
                    ...
                    title:
                        AppLocalizations.of(context)?.translate("Continue") ??
                            "Continue",
                    onTap: () {
                      BlocProvider.of<HomePageCubit>(context, listen: false)
                          .delUnlikeApi(context);
                    },
                  ),
                ],
              ),
                            ...
                            onTap: () {
                              BlocProvider.of<HomePageCubit>(context,
                                  ...
                                  .filterHome(
                                      ...
                                      minage: ageRangeValues.start
                                          .round()
                                          .toString(),
                                      radiusSearch: filterRadius.toString(),
                                      searchPreference: editPro
                                          .maleFemaleBoth(searchPreference),
                                      uid: uid,
                        ...
                        children: [
                          ...
                          Row(
                            children: [
                              ...
                              const Spacer(),
                              Text(
                                "${ageRangeValues.start.round()} - ${ageRangeValues.end.round()}",
                                style: Theme.of(context).textTheme.bodySmall,
                              ),
                            ],
                          ),
                                ...
                                children: [
                                  for (int a = 0;
                                    ...
                                    Builder(builder: (context) {
                                      ...
                                      return InkWell(
                                        onTap: () {
                                          if (languageList.contains(data.id) ==
                                              true) {
                          ...
                          Column(
                            ...
                            children: [
                              ...
                              Wrap(
                                ...
                                children: [
                                  for (int a = 0;
                                      a < editPro.relationShip!.length;
                                      a++)
                                    Builder(builder: (context) {
                                      Goallist data = editPro.relationShip![a];
                                      return InkWell(
                                        onTap: () {
                                          selectRelationShip =
                                              int.parse(data.id.toString());
